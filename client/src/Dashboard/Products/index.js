...
import { Collapse, Select, Form, Input, Button, Row, Col } from 'antd';
...
class Product extends Component {
	...
	render() {
		...
		const catgoriesError = isFieldTouched('categoryCategoryID') && getFieldError('categoryCategoryID');
						...
						<Panel header="Add Product" key="1">
							<Form onSubmit={this.handleSubmit} className="d-flex justify-content-between">
											...
											>
												{getFieldDecorator('productFullName', {
													rules: [
														{
															required: true,
															message: 'Please input your productfullname!'
														}
													]
												})(<Input placeholder="Product Full Name" />)}
											</Form.Item>
										</div>
									</Col>
								</Row>
								<Row>
									<Col span={24 / 4}>
										<Form.Item
											validateStatus={processorError ? 'error' : ''}
											help={processorError || ''}
											label="Processor"
										>
										...
										<Form.Item
											validateStatus={cameraError ? 'error' : ''}
											help={cameraError || ''}
											label="Camera"
										>
								...
								<Row>
									<Col span={24 / 4}>
										<Form.Item
											validateStatus={batteryError ? 'error' : ''}
											help={batteryError || ''}
											label="Battery"
										>
...
const mapDispatchToProps = (dispatch) => {
	return {
		getCategoriesActionStart: () => {
			dispatch(getCategoriesActionStart());
		},
		productStart: (data) => {
			dispatch(productStart(data));
		}
	};
};
